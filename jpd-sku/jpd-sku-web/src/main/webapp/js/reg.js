...
var Reg = {
		...
		isUserEmailExist:function(){
			...
			$.post("/user/isUserEmailExist",{userEmail:userEmail},function(data){
				var json = eval("(" + data + ")");
				if(json.msg){
		...
		doReg:function(){
			...
			if(userflag && passwordflag && telephoneflag){
				$.post("/user/register",{userEmail:userEmail,password:password,userTelephone:userTelephone,userQq:userQq},function(data){
					var json = eval("(" + data + ")");
					if(json.msg){
